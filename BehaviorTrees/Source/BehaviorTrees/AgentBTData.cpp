...
AgentBTData::AgentBTData(AgentBTData &&agentdata)
	...
	m_blackboard(std::move(agentdata.m_blackboard))
{
}
...
{
	MY_ASSERT_WITH_VALUES(m_blackboard != nullptr,
		"Agent's Local BlackBoard Not Initialized\nAgent Name: %s", 
		this->GetName().c_str());

	return static_cast<AgentAbstractData *>(m_blackboard.get());
}

/*--------------------------------------------------------------------------*
Name:           GetGameObject

Description:    Get game object pointer (TinyBlackBoard).
...
Description:    Initialize default local blackboard (AgentAbstractData).
...
{
	// initialize custom data for AgentAbstractClass class
	if (!m_blackboard)
		InitialLocalBlackBoard<AgentAbstractData>();
}

/*--------------------------------------------------------------------------*
Name:           InitialTinyBlackBoard

Description:    Initialize local blackboard (TinyBlackBoard).

Arguments:      npc:			game object associated to agent.

Returns:        None.
*---------------------------------------------------------------------------*/
void AgentBTData::InitialTinyBlackBoard(GameObject *npc)
{
	// initialize custom data for TinyBlackBoard class
...
{
	GetAgentAbstractData()->m_isdeleted = true;
}

/*--------------------------------------------------------------------------*
Name:           IsMarkForDeletion

Description:    Check if the agent is marked for deletion.

Arguments:      None.

Returns:        bool:		true: agent is marked for deletion.
							false: agent isn't marked for deletion.
*---------------------------------------------------------------------------*/
bool AgentBTData::IsMarkForDeletion(void)
{
	return GetAgentAbstractData()->m_isdeleted;
}
...
void AgentBTData::Execute(float dt)
{
	...
	if (m_blackboard)
		m_blackboard->OnMessage();

	// if reset flag is set, reset the whole tree
	if (m_blackboard->m_isreset)
	{
		Enable();
		m_blackboard->m_isreset = false;
