...
namespace BT
{
	...
	public:
		...
		// Get local blackboard
		template <typename T>
		T *GetLocalBlackBoard(void);

		// Get user defined data (AgentAbstractData)
		AgentAbstractData *GetAgentAbstractData(void);

		// Get game object pointer (TinyBlackBoard)
		GameObject *GetGameObject(void);

		// Initial user defined data 
		// (only accept derived struct from AgentAbstractData)
		template <typename T>
		void InitialLocalBlackBoard(void);

		// Initialize default local blackboard (AgentAbstractData)
		void InitialAgentAbstractData(void);
		// Initialize tiny's local blackboard (TinyBlackBoard)
		void InitialTinyBlackBoard(GameObject *npc);
		...
		// Check if the agent is marked for deletion
		bool IsMarkForDeletion(void);
	...
	private:
		...
		std::unique_ptr<AgentAbstractData> m_blackboard;		// local blackboard for each agent
	...
	Description:    Get local blackboard.
	...
	*---------------------------------------------------------------------------*/
	template <typename T>
	void AgentBTData::InitialLocalBlackBoard(void)
	{
		m_blackboard = std::make_unique<T>();
	}
